...
const YETIToken = artifacts.require("./YETIToken.sol");
...
const WJLP = artifacts.require("./AssetWrappers/WJLP/WJLP.sol");
...
class DeploymentHelper {
  ...
  static async deployAssetWrappers() {
    ...
    //   return this.deployAssetWrappersHardhat()
    // }
  }
  ...
  static async connectCoreContracts(contracts, YETIContracts) {
    ...
    await contracts.whitelist.addCollateral(contracts.weth.address, "1000000000000000000", contracts.priceFeedETH.address, 18, contracts.PriceCurveETH.address, false);
    await contracts.whitelist.addCollateral(contracts.wavax.address, "1000000000000000000", contracts.priceFeedAVAX.address, 18, contracts.PriceCurveAVAX.address, false);
    await contracts.whitelist.addCollateral(contracts.wJLP.address, "1000000000000000000", contracts.priceFeedJLP.address, 18, contracts.PriceCurveJLP.address, true);
  }
  ...
  static async deployExtraCollateral(contracts, params) {
    ...
    await contracts.whitelist.addCollateral(newToken.address, ratio, newPriceFeed.address, decimals, newPriceCurve.address, false);
