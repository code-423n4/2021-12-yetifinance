...
contract WJLP is ERC20_8, IWAsset {
    ...
    function getPendingRewards(address _for) external override returns
        ...
        uint pending = user.amount.mul(accJoePerShare).div(1e12).sub(user.rewardDebt);

        address[] memory tokens = new address[](1);
        uint[] memory amounts = new uint[](1);
        tokens[0] = address(JLP);
        amounts[0] = unclaimed.add(pending);
